...
class EventSource {
  ...
  constructor(domNode) {
    ["pointerdown", "pointerup", "pointermove", "keydown"].forEach(
      (eventType) => {
  ...
  _promiseForEvent(eventType) {
    assert(
      this._promisesByEventType.has(eventType),
      `not supported: ${eventType}`
    );
...
export default { EventSource, or, and, run, loop, log };
